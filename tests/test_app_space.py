...
def test_get_app_space_id_success(capsys):
    client = ConfigClient()
    assert client is not None
    assert client.credentials.get('serviceAccountId') is not None
...
def test_get_app_space_by_name_wrong_name(capsys):
    ...
    response = client.get_app_space_by_name(service_account.customer_id, app_space_name)
    captured = capsys.readouterr()
    assert response is None
...
def test_get_app_space_name_success(capsys):
    ...
    app_space = client.get_app_space_by_name(service_account.customer_id, app_space_name)
    captured = capsys.readouterr()
