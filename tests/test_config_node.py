...
def test_update_consent_config_node_success(capsys):
    client = ConfigClient()
    assert client is not None

    right_now = str(int(time.time()))
    config_node_id = data.get_consent_config_node_id()
    response = client.read_config_node(config_node_id)
    assert response is not None

    consent_config = data.get_consent_config()
    config_node_response = client.update_consent_config_node(response.id,
                                                             response.etag,
                                                             "Automation "+right_now,
                                                             "description "+right_now,
                                                             consent_config,
...
def test_update_consent_config_node_empty(capsys):
    client = ConfigClient()
    assert client is not None
...
def test_update_consent_config_node_exception(capsys):
    client = ConfigClient()
    assert client is not None
