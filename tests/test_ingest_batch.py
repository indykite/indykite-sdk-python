...
def test_ingest_batch_identity_success(client, external_ids):
    external_id, external_id2 = external_ids
...
def test_ingest_batch_relationship_success(client):
    type = "CAN_USE"
...
def test_delete_batch_node_properties(client, external_ids):
    external_id, external_id2 = external_ids
...
def test_delete_record_relationship_property(client):
    type = "CAN_USE"
    source_match = client.node_match("vehicle-1", "Vehicle")
    assert isinstance(source_match, model_pb2.NodeMatch)
    target_match = client.node_match("lot-1", "ParkingLot")
...
def test_data_node_error(client, capsys):
...
def test_data_relationship_error(client, capsys):
...
def test_delete_batch_node_tags(client, external_ids):
    external_id, external_id2 = external_ids
    ingest_property = client.ingest_property("custom1", "478")
    assert isinstance(ingest_property, ikg_pb2.Property)
    properties = [ingest_property]
    ...
    node_match1 = client.node_match(external_id, "Person")
    node_match2 = client.node_match(external_id2, "Person")
    node_tag = client.node_tag_match(node_match1, ["User"])
    node_tag2 = client.node_tag_match(node_match2, ["Employee"])
    response = client.batch_delete_node_tags([node_tag, node_tag2])
    assert isinstance(response, BatchDeleteNodeTagsResponse)
    delete_batch_node = client.batch_delete_nodes([node_match1, node_match2])
    assert isinstance(delete_batch_node, BatchDeleteNodesResponse)


def test_delete_batch_node_tags_error(client, external_ids, capsys):
    external_id, external_id2 = external_ids
    ingest_property = client.ingest_property("custom1", "478")
    assert isinstance(ingest_property, ikg_pb2.Property)
    properties = [ingest_property]
    ...
    response = client.batch_upsert_nodes([node1, node2])
    captured = capsys.readouterr()
    assert "tag 'Person' cannot be manipulated" in captured.err


def test_delete_batch_node_tags_wrong_tags(client, external_ids, capsys):
    external_id, external_id2 = external_ids
    ingest_property = client.ingest_property("custom1", "478")
    assert isinstance(ingest_property, ikg_pb2.Property)
    properties = [ingest_property]
    ...
    node_match1 = client.node_match(external_id, "Person")
    node_match2 = client.node_match(external_id2, "Person")
    node_tag = client.node_tag_match(node_match1, ["Person"])
    node_tag2 = client.node_tag_match(node_match2, ["Person"])
    response = client.batch_delete_node_tags([node_tag, node_tag2])
    captured = capsys.readouterr()
    assert "tag 'Person' cannot be manipulated" in captured.err
    delete_batch_node = client.batch_delete_nodes([node_match1, node_match2])
