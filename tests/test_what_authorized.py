...
def test_what_authorized_token_wrong_token(client, capsys):
    ...
    resource_types = [
        WhatAuthorizedResourceTypes("TypeNamePrime", actions),
...
def test_what_authorized_token_empty(client):
    ...
    input_params = {}
    res = [pb2.WhatAuthorizedRequest.ResourceType(type=r.type, actions=list(r.actions)) for r in resource_types]
    subject = pb2_model.Subject(access_token=str(access_token))
...
def test_what_authorized_token_success(client):
    ...
    input_params = {}
    res = [pb2.WhatAuthorizedRequest.ResourceType(type=r.type, actions=list(r.actions)) for r in resource_types]
    subject = pb2_model.Subject(access_token=str(token))
...
def test_what_authorized_identity_node_empty(client):
    ...
    input_params = {}
    subject = pb2_model.Subject(digital_twin_id=pb2_model.DigitalTwin(id=str(identity_node_id)))
...
def test_what_authorized_property_empty(client):
    ...
    input_params = {}
    subject = pb2_model.Subject(external_id=pb2_model.ExternalID(type=str(node_type), external_id=str(external_id)))
