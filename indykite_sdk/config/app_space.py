...
def read_app_space_by_id(self, app_space_id):
    ...
    try:
        response = self.stub.ReadApplicationSpace(pb2.ReadApplicationSpaceRequest(id=str(app_space_id)))
...
def read_app_space_by_name(self, customer_id, app_space_name):
    ...
    try:
        response = self.stub.ReadApplicationSpace(
            pb2.ReadApplicationSpaceRequest(name=UniqueNameIdentifier(location=customer_id, name=app_space_name)),
        )
    except Exception as exception:
        return logger.logger_error(exception)
...
def list_app_spaces(self, customer_id, match=[]):
    ...
    try:
        streams = self.stub.ListApplicationSpaces(
            pb2.ListApplicationSpacesRequest(customer_id=customer_id, match=match),
        )
    except Exception as exception:
        return logger.logger_error(exception)
...
def delete_app_space(self, app_space_id, etag):
    ...
    try:
        response = self.stub.DeleteApplicationSpace(
            pb2.DeleteApplicationSpaceRequest(id=app_space_id, etag=wrappers.StringValue(value=etag)),
        )
    except Exception as exception:
        return logger.logger_error(exception)
