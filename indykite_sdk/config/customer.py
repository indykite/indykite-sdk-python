...
def read_customer_by_id(self, customer_id):
    ...
    try:
        response = self.stub.ReadCustomer(pb2.ReadCustomerRequest(id=str(customer_id)))
...
def read_customer_by_name(self, customer_name):
    ...
    sys.excepthook = logger.handle_excepthook
    try:
        response = self.stub.ReadCustomer(pb2.ReadCustomerRequest(name=str(customer_name)))
    except Exception as exception:
        return logger.logger_error(exception)
