...
def create_consent_config_node(self,
    ...
    sys.excepthook = logger.handle_excepthook
    try:
        if consent_config and consent_config.data_points:
            consent_config.data_points = self.validate_data_points(consent_config.data_points)
        response = self.stub.CreateConfigNode(
            pb2.CreateConfigNodeRequest(
                location=location,
                name=name,
...
def consent_config(self, policy, status, tags=[]):
    """
