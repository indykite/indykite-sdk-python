...
def grant_consent(self, user, consent_id, validity_period):
    ...
    try:
        ...
        response = self.stub.GrantConsent(
            pb2.GrantConsentRequest(user=user_obj, consent_id=consent_id, validity_period=int(validity_period)),
        )
        if not response:
            return None
        return response
...
def revoke_consent(self, user, consent_id):
    ...
    try:
        user_obj = user_validation(user)
        response = self.stub.RevokeConsent(pb2.RevokeConsentRequest(user=user_obj, consent_id=consent_id))
    except Exception as exception:
        return logger.logger_error(exception)
